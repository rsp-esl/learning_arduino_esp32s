...
#ifndef DE_RE_PIN
  #define DE_NOT_RE_PIN                     NOT_A_PIN                          //default digital pin for control MAX485 DE/RE lines (connect DE & /RE together to this pin)
#endif
...
class SDM {
  public:
    ...
    SDM( HardwareSerial& serial, 
         int rx_pin,
         int tx_pin,
         int de_not_re_pin = DE_NOT_RE_PIN, 
         long baud = SDM_UART_BAUD, 
         int config = SDM_UART_CONFIG );
  ...
  private:
    ...
    int _rx_pin, _tx_pin, _de_not_re_pin = DE_NOT_RE_PIN;
