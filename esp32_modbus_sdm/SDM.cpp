...
// This is a modified version of the Arduino library for SDM Enery Meter.
// see: https://github.com/reaper7/SDM_Energy_Meter/

SDM::SDM( HardwareSerial& serial, 
         int rx_pin,
         int tx_pin,
         int de_not_re_pin, 
         long baud, 
         int config ) : sdmSer(serial) 
{
  this->sdmSer = serial;
  this->_rx_pin = rx_pin;
  this->_tx_pin = tx_pin;
  this->_baud = baud;
  this->_config = config;
  this->_de_not_re_pin = de_not_re_pin;
}
...
void SDM::begin(void) {
  ...
  if ( _de_not_re_pin != NOT_A_PIN ) { //set output pin mode for DE/RE pin when used (for control MAX485)
     pinMode( _de_not_re_pin, OUTPUT );
  }
}
...
float SDM::readVal( uint16_t reg, uint8_t node ) {
  ...
  if (_de_not_re_pin != NOT_A_PIN) {                                            //transmit to SDM  -> DE Enable, /RE Disable (for control MAX485)
    digitalWrite(_de_not_re_pin, HIGH);
  }
  ...
  if (_de_not_re_pin != NOT_A_PIN) {                                            //receive from SDM -> DE Disable, /RE Enable (for control MAX485)
    digitalWrite( _de_not_re_pin, LOW );
  }
